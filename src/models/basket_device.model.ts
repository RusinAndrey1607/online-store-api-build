...
class BasketDeviceModel {
  ...
  async delete(body: { device_id: number; basket_id: number }) {
    return await pool.query(
      `DELETE FROM basket_devices WHERE device_id = ${body.device_id} AND basket_id=${body.basket_id};`
    );
  }
  ...
  async create(body: { device_id: number; basket_id: number }) {
    return (
      await pool.query(
        `INSERT INTO basket_devices (device_id,basket_id) VALUES (${body.device_id},${body.basket_id}) RETURNING id, basket_id,device_id;`
      )
    ).rows[0];
  }
}
